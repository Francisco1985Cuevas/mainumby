<div class="form-group">
	{!! Form::label('idregion', '#C&oacute;digo Identificador') !!}
	{!! Form::label('idregion', '(Campo Autogenerado)', ['class' => 'text-xs']); !!}
    ...
    <div class="form-group">
        ...
        <select id="pais_id" name="pais_id" class="form-control" required>
            ...
            @foreach ($listaPaises as $pais)
                ...
                @else
                    <option value="{{ $pais->id }}" {{ $pais->id == $region->pais->id ? 'selected':'' }} >{{ $pais->nombre }}</option>
                @endif
            @endforeach
        </select>
    </div>
...
<div class="form-group">
    ...
    {!! Form::label('descripcion', '(Campo opcional)', ['class' => 'text-xs']); !!}
    <textarea id="descripcion" name="descripcion" class="form-control" rows="5" cols="5">
        @isset($region)
			{{ $region->descripcion }}
		@endisset
    </textarea>
