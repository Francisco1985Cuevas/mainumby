<div class="form-group">
	{!! Form::label('idmoneda', '#C&oacute;digo Identificador') !!}
	{!! Form::label('idmoneda', '(Campo Autogenerado)', ['class' => 'text-xs']); !!}
    ...
    <div class="form-group">
        ...
        <select id="pais_id" name="pais_id" class="form-control" required>
            ...
            @foreach ($listaPaises as $pais)
                ...
                @else
                    <option value="{{ $pais->id }}" {{ $pais->id == $moneda->pais->id ? 'selected':'' }} >{{ $pais->nombre }}</option>
                @endif
            @endforeach
        </select>
    </div>
